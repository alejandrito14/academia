...
if(!isset($_GET['idpublicidad'])){
	...
	$obtenerorden=$emp->ObtenerUltimoOrdenpublicidad();
	...
	$emp->idpublicidad = $idpublicidad;

	//Realizamos la consulta en tabla empresas
	$result_PUBLICIDAD = $emp->buscarpublicidad();
	$result_PUBLICIDAD_row = $db->fetch_assoc($result_PUBLICIDAD);

	//Cargamos en las variables los datos de las empresas

	//DATOS GENERALES
	$tituloPUBLICIDAD = $f->imprimir_cadena_utf8($result_PUBLICIDAD_row['titulo']);
	//$descripcion = $f->imprimir_cadena_utf8($result_PUBLICIDAD_row['descripcion']);
	
	$foto = $f->imprimir_cadena_utf8($result_PUBLICIDAD_row['imagen']);
	$orden = $f->imprimir_cadena_utf8($result_PUBLICIDAD_row['orden']);
	$estatus = $f->imprimir_cadena_utf8($result_PUBLICIDAD_row['estatus']);
	...
	if($foto==""){
		...
		$ruta="catalogos/publicidad/imagenes/".$_SESSION['codservicio']."/$foto";
	}

	$col = "col-md-12";
	$ver = "";
		$titulo='EDITAR PUBLICIDAD';
...
<form id="f_PUBLICIDAD" name="f_PUBLICIDAD" method="post" action="">
	<div class="card">
		<div class="card-body">
			...
			<div style="float: right;">
				...
				<?php
					...
					$bt->funcion = "var resp=MM_validateForm('v_titulo','','R'); if(resp==1){ Guardarpublicidad('f_PUBLICIDAD','catalogos/publicidad/vi_publicidad.php','main','$idmenumodulo');}";
				...
				<button type="button" onClick="aparecermodulos('catalogos/publicidad/vi_publicidad.php?idmenumodulo=<?php echo $idmenumodulo;?>','main');" class="btn btn-primary" style="float: right; margin-right: 10px;"><i class="mdi mdi-arrow-left-box"></i> LISTADO DE PUBLICIDAD </button>
				<div style="clear: both;"></div>
				
				<input type="hidden" id="id" name="id" value="<?php echo $idpublicidad; ?>" />
			</div>
			<div style="clear: both;"></div>
		</div>
	</div>
					...
					<div class="tab-content tabcontent-border">
						<div class="tab-pane active show" id="generales" role="tabpanel">
								    ...
								    <div class="card" style="width: 18rem;margin: auto;margin-top: 3em;">
								        ...
								        <div class="card-body">
								            ...
								            <div class="form-group">
								                ...
								                <input type="file" class="form-control-file" name="image" id="image" onchange="SubirImagenPUBLICIDAD()">
								            </div>
							...
							<div class="form-group m-t-20">
								<label>*TITULO:</label>
							<input type="text" class="form-control" id="v_titulo" name="v_titulo" value="<?php echo $tituloPUBLICIDAD; ?>" title="TITULO" placeholder='TITULO'>
							</div>
...
<script>
	var ruta='<?php echo $ruta;?>';
        ...
        $.ajax({
            url: 'catalogos/publicidad/upload.php',
            type: 'post',
            data: formData,
            contentType: false,
            processData: false,
