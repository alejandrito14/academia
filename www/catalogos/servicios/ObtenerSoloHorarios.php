...
try
{
	...
	for ($i=0; $i <count($horariosdeseados) ; $i++) { 
			...
			for ($h=0; $h <count($zonasarray); $h++) { 
				# code...
				$horarioservicio->idzona=0;
				$zonas->idzona=0;
				/*$datoszona=$zonas->ObtenerZona();*/
				array_push($arraydatoszona,'Zona 0');

			for ($i=0; $i <count($arreglodiasfechas) ; $i++) {

				$arreglodiasfechas[$i]['nombrezona']='Zona 0';
				$arreglodiasfechas[$i]['idzona']=0;
				$arreglodiasfechas[$i]['color']='';
...
//$arreglodiasfechas[$i]['horasposibles'][0][$j]['disponible']=0;
		...
		if (count($arraydiaszonas)>0) {
			...
			usort($arraydiaszonas, function ($a, $b) {
