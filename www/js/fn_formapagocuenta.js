function Guardarformapagocuenta(form,regresar,donde,idmenumodulo)
{
	...
	{			
		...
		setTimeout(function(){
				  $.ajax({
					url:'catalogos/formapagocuenta/ga_formapago.php', //Url a donde la enviaremos
...
function BorrarFormapafocuenta(idformapago,idmenumodulo,regresar,donde) {
	if(confirm("\u00BFEstas seguro de querer realizar esta operaci\u00f3n?"))
	{
var datos='idformapago='+idformapago;
	$.ajax({
		url:'catalogos/formapagocuenta/borrarFormapago.php', //Url a donde la enviaremos
	  type:'POST', //Metodo que usaremos
	  data: datos, //Le pasamos el objeto que creamos con los archivos
	  error:function(XMLHttpRequest, textStatus, errorThrown){
			var error;
