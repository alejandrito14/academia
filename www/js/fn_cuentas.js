function GuardarCuenta(form,regresar,donde,idmenumodulo)
{
	...
	{			
		...
		setTimeout(function(){
				  $.ajax({
					url:'catalogos/cuentas/ga_cuentas.php', //Url a donde la enviaremos
...
function BorrarCuenta(idcuenta,idmenumodulo,regresar,donde) {
	if(confirm("\u00BFEstas seguro de querer realizar esta operaci\u00f3n?"))
	{
var datos='idcuenta='+idcuenta;
	$.ajax({
		url:'catalogos/cuentas/borrarCuenta.php', //Url a donde la enviaremos
	  type:'POST', //Metodo que usaremos
	  data: datos, //Le pasamos el objeto que creamos con los archivos
	  error:function(XMLHttpRequest, textStatus, errorThrown){
			var error;
