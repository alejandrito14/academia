...
*/
	...
	WHERE usuarios_servicios.idservicio=servicios.idservicio AND  usuarios.tipo=3 AND usuarios_servicios.aceptarterminos=1
) AS cantalumnos
			from servicios
  ...
  width:100px;
 		...
 		<table class="table  table table-striped table-bordered table-responsive vertabla" border="1" style="">
		  ...
		  <tr bgcolor="#3B3B3B" style="color: #FFFFFF; text-align: left;">
		    ...
		    <th>No. DE HORAS</th>
		    <th>No. DE ALUMNOS QUE TOMAN LA CLASE</th>
		    <th>ID</th>
		    <th>ALUMNO</th>
	...
	$asignacion->idusuario=$obtenerservicios[$i]->idusuarios;
	 ...
	 $idcoach=$obtenerservicios[$i]->idusuarios;
			...
			$objeto=array('idusuariocoach'=>$idcoach,'coach'=>$nombrecoach,'tipocomision'=>$tipomontopago[0]->tipopago,'monto'=>$tipomontopago[0]->monto,'montocomision'=>0,'montopagocoach'=>$montopagocoach,'cantidadhorarios'=>$cantidadhorarios);
		 			...
		 			$folio=$obtenernotapago[0]->folio;
			 			...
			 			if (count($arraycoachcomision)>0) {
			 				for ($l=0; $l < count($arraycoachcomision); $l++) { 
			 					...
			 					$montocomision=$asignacion->CalcularMontoPago2($arraycoachcomision[$l]['tipocomision'],$arraycoachcomision[$l]['monto'],$montopagocondescuento,$arraycoachcomision[$l]['cantidadhorarios']);
...
$fechafin=date('Y-m-d',strtotime($fechafin));
		 			...
		 			if($fechapago2>=$fechainicio && $fechapago2<=$fechafin) {
						...
						$objeto=array(
								...
								'cantidadhorarios'=>$array[$k]->cantidadhorarios,
								'cantalumnos'=>$array[$k]->cantalumnos
				     ...
				     <td><?php echo $pagosservicios[$l]['nombreservicio']?>
				      ...
				      <td><?php echo $pagosservicios[$l]['fechahoramin'].' / '. $pagosservicios[$l]['fechahoramax'];?>
				 	 </td>

				     <td><?php echo $pagosservicios[$l]['cantidadhorarios'];?></td>
				    <td><?php echo $pagosservicios[$l]['cantalumnos'];?></td>
