...
try
{
	...
	for ($i=0; $i < count($obtenerzonaho); $i++) { 
		...
		//if ($horainicial>=$horaactual) {
		 ...
		 $intervalos=$fechas->intervaloHora($obtenerzonaho[$i]->horainicial,$obtenerzonaho[$i]->horafinal,30);
	...
	for ($i=0; $i <count($horariosdeseados) ; $i++) { 
			...
			$resultado=$f->agruparHorarios($diashoras);
