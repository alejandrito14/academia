...
try
{
	...
	$fechashorasseleccionadas=json_decode($_POST['fechashorasseleccionadas']);
	...
	if ($tipousuario==0){
		...
		if (count($fechashorasseleccionadas)>0) {
			


			for ($h=0; $h <count($fechashorasseleccionadas); $h++) { $fecha=$fechashorasseleccionadas[$h]->fecha;

				$horas=$fechashorasseleccionadas[$h]->horas;

				for ($k=0; $k <count($horas) ; $k++) { 
						
				$horadividida=explode('-', $horas[$k]);
				$horainicial=$horadividida[0];
				$horafinal=$horadividida[1];
				$emp->dia=date('w',strtotime($fecha));
				$emp->horainiciosemana=$horainicial;
				$emp->horafinsemana=$horafinal;
				$emp->fecha=date('Y-m-d',strtotime($fecha));
				$emp->GuardarHorarioPrevio();
