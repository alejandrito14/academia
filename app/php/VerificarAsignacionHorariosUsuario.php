...
try
{
	...
	$usuarios=explode(',',$_POST['idusuarios']);
	...
	for ($i=0; $i < count($usuarios); $i++) { 
		...
		if ($valor<count($usuarios)) {
			...
			for ($k=0; $k <count($obtenerHorarios) ; $k++) { 
				...
				$serviciosasignados->fecha=date('Y-m-d',strtotime($fecha));
				...
				for ($l=0; $l < count($arraydiaselegidos); $l++) { 
					...
					$cruzahorario=$serviciosasignados->EvaluarHorarioFechaServicio(date('Y-m-d',strtotime($fechae)),$horainiciale,$horafinale);
			...
			if ($secruza>0) {
				...
				if (count($usuariosnoagregados)>0) {
					

					for ($l=0; $l <count($usuariosnoagregados) ; $l++) { 
						if ($usuariosnoagregados[$l]->idusuarios==$usua->idusuarios) {
							
							$servicru=$usuariosnoagregados[$l]->servicioscruzados;

							if (count($servicru)>0) {

								for ($n=0; $n <count($servicioscruzados) ; $n++) {
								$existe=0;
								for ($m=0; $m <count($servicru) ; $m++) { 

										if ($servicru[$m]->idservicio==$servicioscruzados[$n]->idservicio) {
											...
											if ($existe==0) {
												...
												array_push($usuariosnoagregados[$l]->servicioscruzados,$servicioscruzados[$n]);
											}
								...
								array_push($usuariosnoagregados[$l]->servicioscruzados, $servicioscruzados);
