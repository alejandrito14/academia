...
try
{
	...
	for ($i=0; $i <count($horariosdeseados) ; $i++) { 
			...
			for ($i=0; $i <count($arreglodiasfechas) ; $i++) {
				...
				$objeto=array('fecha'=>$fecha,'horas'=>[]);

				
				if (count($arrayfechashoras)>0) {
					# code...
				$noencontrada=0;
				foreach ($arrayfechashoras as $item) {
					...
					if ($noencontrada==0) {
					    	array_push($arrayfechashoras, $objeto);
					    }
			...
			$arreglodiasfechas[$i]['horasposibles'][0][$j]['disponible']=0;
							...
							$arreglodiasfechas[$i]['horasposibles'][0][$j]['disponible']=1;
								$agregar=$horainicial.'-'.$horafinal;
						
						for ($l=0; $l <count($arrayfechashoras) ; $l++) { 
							
							if ($arrayfechashoras[$l]['fecha']==$fecha) {

								$encontrado=0;
								for ($m=0; $m < count($arrayfechashoras[$l]['horas']); $m++) { 
								

									if ($arrayfechashoras[$l]['horas'][$m]==$agregar) {
										$encontrado=1;
									}
								}
								...
								if ($encontrado==0) {
									...
									if ($agregar!='' && $horafinal!='') {
										array_push($arrayfechashoras[$l]['horas'], $agregar);
									}
	...
	usort($arrayfechashoras, 'compararFechas');
	...
	$respuesta['arrayfechahora']=$arrayfechashoras;
